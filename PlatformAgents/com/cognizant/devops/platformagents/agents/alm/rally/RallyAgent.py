...
class RallyAgent(BaseAgent):
    def process(self):
        ...
        specificProjectsList = self.config.get('dynamicTemplate', {}).get('specificProjectsList', None)
        ...
        for i in range(0, len(specificProjectsList)):
            
            project_id = specificProjectsList[i]
       
            
            
            
            artifactListUrl = baseUrl+"artifact?project="+project_id+"&query=(LastUpdateDate > "+lastUpdated+")"
